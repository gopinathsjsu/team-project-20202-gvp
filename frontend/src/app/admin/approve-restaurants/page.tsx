...
export default function ApproveRestaurantsPage() {
  ...
  useEffect(() => {
    const fetchUnapprovedRestaurants = async () => {
      try {
        const response = await fetch("http://localhost:8000/api/restaurants/admin/unapproved/", {
          headers: {
  ...
  const handleApprove = async (restaurantId: number) => {
    try {
      const response = await fetch(`http://localhost:8000/api/restaurants/admin/approve/${restaurantId}/`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${tokens?.access}`,
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-900">Approve Restaurants</h1>
          <Link 
            href="/admin" 
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          {loading ? (
            ...
            <div className="text-center py-4 text-gray-500">No restaurants pending approval</div>
          )}
        </div>
      </main>
    </div>
